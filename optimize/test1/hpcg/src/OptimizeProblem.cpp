...
// 定义函数指针类型
typedef int (*ComputeSPMVFunction)(const SparseMatrix & A, Vector & x, Vector & y);
...
int OptimizeProblem(SparseMatrix & A, CGData & data, Vector & b, Vector & x, Vector & xexact) {
  ...
  // 将函数指针转换为void*存储
  ComputeSPMVFunction spmv_func = ComputeSPMV_opt;
  A.optimizationData = reinterpret_cast<void*>(spmv_func);
